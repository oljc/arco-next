...
title:
  ...
  en-US: Status
description:
  zh-CN: 链接的状态分为 `normal` - **正常（默认）**、`success` - **成功**、`warning` - **警告**、`danger` - **危险**四种。
  en-US: "The status of the link is divided into four types: `normal` (default), `success`, `warning` and `danger`."
```
