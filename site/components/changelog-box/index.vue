<template>
  ...
  >
    <div class="changelog-box-filter">
      <div class="changelog-box-filter-title">{{ t('changelogBox.filter') }}:</div>
      <Grid :cols="5" :col-gap="20">
        ...
        <GridItem :span="2">
          ...
          <RangePicker v-else :model-value="[start, end]" @change="onRangePickerChange" />
        </GridItem>
        <GridItem :span="2">
          ...
          <div class="changelog-item">
            ...
            <div class="changelog-content">
              <ul v-if="item.extra && item.extra.length">
                <li v-for="(text, index) of item.extra" :key="index" v-html="text" />
              </ul>
              <section v-for="content of item.list" :key="content.type">
                ...
                <ul class="changelog-content-list">
                  <li v-for="(text, index) of content.list" :key="index" v-html="text" />
                </ul>
              </section>
            </div>
          </div>
...
const compareVersion = (v1: string, v2: string) => {
  ...
  for (let i = 0; i < 3; i++) {
    if (array1[i] !== array2[i]) {
      return parseInt(array1[i] ?? '0', 10) > parseInt(array2[i] ?? '0', 10) ? 1 : -1;
    }
  }
  return 0;
};
...
const start = ref('');
const end = ref('');
const type = ref(['feature', 'bugfix', 'enhancement', 'style', 'typescript', 'attention']);
...
const versions = computed(() => props.changelog.map((item: any) => item.version));
